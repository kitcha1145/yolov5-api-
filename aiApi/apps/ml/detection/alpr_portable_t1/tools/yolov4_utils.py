...
def process_box(b, h, w, threshold, _label):
    max_indx = np.argmax(b[4])
    max_prob = b[max_indx]
    label = _label[max_indx]
...
def darkflow_format(image, bboxes, classes, threshold=0.5, index=None, x_scale=1.0, y_scale=1.0):
    boxesInfo = list()
...
def draw_bbox(image, bboxes, classes, show_label=True, index=None):
    """
...
def postprocess_bbbox_v3(pred_bbox, ANCHORS, STRIDES, XYSCALE=[1, 1, 1]):
    ...
    for i, pred in enumerate(pred_bbox):
        ...
        # print(f'xy_grid[{i}]: {len(xy_grid)}')
        xy_grid = np.expand_dims(np.stack(xy_grid, axis=-1), axis=2)  # [gx, gy, 1, 2]
        # print(f'xy_grid[{i}]: {len(xy_grid)}')

        # print(tf.expand_dims(xy_grid, axis=0))
...
def postprocess_boxes(pred_bbox, org_img_shape, input_size: list, score_threshold):
    ...
    resize_ratio_x = input_size[0] / org_w
    resize_ratio_y = input_size[1] / org_h
    # min(input_size / org_w, input_size / org_h)

    dw = (input_size[0] - resize_ratio_x * org_w) / 2
    dh = (input_size[1] - resize_ratio_y * org_h) / 2
