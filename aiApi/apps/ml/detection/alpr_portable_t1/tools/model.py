...
slim = tf_slim
from mutils.layer_utils import conv2d, darknet53_body, yolo_block, upsample_layer
...
class yolov3_16(object):
    ...
    def reorg_layer(self, feature_map, anchors):
        ...
        grid_size = feature_map.get_shape().as_list()[1:3] if self.use_static_shape else tf.shape(feature_map)[
                                                                                         1:3]  # [13, 13]
        # the downscale ratio in height and weight
        ratio = tf.cast(self.img_size / grid_size, tf.float16)
        # rescale the anchors to the feature_map
        # NOTE: the anchor is in [w, h] format!
        rescaled_anchors = [(anchor[0] / ratio[1], anchor[1] / ratio[0]) for anchor in anchors]
        ...
        x_y_offset = tf.concat([x_offset, y_offset], axis=-1)
        # shape: [13, 13, 1, 2]
        x_y_offset = tf.cast(tf.reshape(x_y_offset, [grid_size[0], grid_size[1], 1, 2]), tf.float16)
    ...
    def loss_layer(self, feature_map_i, y_true, anchors):
        ...
        ratio = tf.cast(self.img_size / grid_size, tf.float16)
        # N: batch_size
        N = tf.cast(tf.shape(feature_map_i)[0], tf.float16)
        ...
        # the calculation of ignore mask if referred from
        # https://github.com/pjreddie/darknet/blob/master/src/yolo_layer.c#L179
        ignore_mask = tf.TensorArray(tf.float16, size=0, dynamic_size=True)
        ...
        def loop_body(idx, ignore_mask):
            ...
            best_iou = tf.reduce_max(iou, axis=-1)
            # shape: [13, 13, 3]
            ignore_mask_tmp = tf.cast(best_iou < 0.5, tf.float16)
            # finally will be shape: [N, 13, 13, 3]
            ignore_mask = ignore_mask.write(idx, ignore_mask_tmp)
            return idx + 1, ignore_mask
        ...
        # box with smaller area has bigger weight. This is taken from the yolo darknet C source code.
        # shape: [N, 13, 13, 3, 1]
        box_loss_scale = 2. - (y_true[..., 2:3] / tf.cast(self.img_size[1], tf.float16)) * (
                y_true[..., 3:4] / tf.cast(self.img_size[0], tf.float16))
